...
describe('test cursor emptiness', () => {
	...
	it('cursor and limit test', async () => {
		...
		const p10 = getDocs(query(colRef, offset(1))).then(querySnapshot => {
			expect(querySnapshot.docs.length).toBe(3)
		})
		const p11 = getDocs(query(colRef, offset(2))).then(querySnapshot => {
			expect(querySnapshot.docs.length).toBe(2)
		})
		await Promise.all([p5, p6, p7, p8, p9, p10, p11])
	})
})
